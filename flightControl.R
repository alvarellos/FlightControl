...
# List the most popular airports
mostpopular
...
# For the purposes of this question, treat the group of the 200 least popular airports 
...
# Quiz 12
# How many flights landed at Ronald Reagan Washington National or Washington Dulles Airport in 2008? 
...
# Quiz 14
# How many flights departed altogether from ATL, AUS, and BDL during the months of July 2008 through October 2008?
sum(tapply(myDF$DepDelay,list(myDF$Origin,myDF$Month),length)[c("ATL","AUS","BDL"),7:10])

# Can you create a table with 3 rows and 12 columns, showing the monthly 
# counts for flights departing from ATL, ORD, and DFW?
tapply(myDF$Origin, list(myDF$Origin,myDF$Month), length) [c("ATL","ORD","DFW"), ]


# Quiz 15

# Question 1
# How many flights departed altogether from IND or ORD in 2008 
# with a delay of more than 30 minutes each?
sum(tapply(longdelayDF$DepDelay, list(longdelayDF$Origin,longdelayDF$Month), 
           length)[c("IND","ORD"), ])
...
sort(tapply(longdelayDF$DepDelay, longdelayDF$Month,length)/tapply(myDF$DepDelay, myDF$Month,length))


# Quiz 16
# How many flights departed from IND early in the morning?
table(myDF$timeofday,myDF$Origin)["early morning","IND"]

# Another way of doing this is by looking at the table 
table(myDF$TimeOfDay == "early morning" & myDF$Origin == "IND")
